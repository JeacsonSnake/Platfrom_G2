...
def generate_report(results, seg_results, motor_id, fig_path, transient_path):
    ...
    lines.append("## 1. 测试方法")
    lines.append("")
    lines.append("- PID 控制周期为 100ms（10Hz）。")
    lines.append("- `actual` 转速由 GPIO 中断捕获 FG 脉冲周期计算得出（6 PPR，RPM = 10,000,000 / period_us），理论分辨率远高于 1 RPM；`raw` 字段仍保留 200ms PCNT 原始计数以供参考。")
    lines.append("- 对每个目标速度，发送 `cmd_M_<target>_10` 指令，持续约 10 秒。")
    lines.append("- 稳态值取该指令后半段（50% 以后）的实际速度与 PWM duty 平均值。")
    ...
    lines.append("- 异常值过滤：单点速度超过 5400 RPM（电机物理上限 4500 的 120%）视为脉冲计数异常，不参与统计；")
    ...
    lines.append("")
    lines.append("## 7. 测量与采样评估")
    lines.append("")
    lines.append("- **转速测量方法**：`actual` 字段由 GPIO 中断捕获每个 FG 脉冲的周期并做滑动平均得到（6 PPR，RPM = 10,000,000 / period_us）。")
    lines.append("  - 在 1 µs 定时器分辨率下，即使 4500 RPM（周期约 2222 µs）也有优于 0.05% 的相对分辨率，低速时分辨率更高。")
    lines.append("  - 因此 `actual` 字段不再受 200ms PCNT 计数 50 RPM 量化的限制，可观察到电机自身的速度抖动。")
    lines.append("- **PCNT 原始计数（raw=.../200ms）**：仍保持 200ms 采样，仅作兼容字段；单脉冲对应 50 RPM，分辨率约 1.1%（@4500 RPM）。")
    lines.append("- **PID 控制周期**：100ms（10Hz），与高精度周期捕获读数匹配，可更快响应速度波动。")
    lines.append("- **香农采样定理角度**：电机机械时间常数较大，速度信号带宽远低于 5Hz，10Hz 控制周期在理论上是足够的。")
    lines.append("- **是否进一步提高控制频率**：")
    lines.append("  - 若目标仅为稳态精度：当前 10Hz 配合周期捕获已足够，无需改动。")
    lines.append("  - 若需进一步精细化启动/切换瞬态：可提高到 20Hz（50ms），但 PCNT 计数分辨率会下降，且需重新整定 PID 参数。")
    lines.append("  - 由于实际转速测量已改为周期法，单纯提高控制频率对读数精度提升有限，主要影响 PID 响应速度。")
    lines.append("")
    ...
    lines.append("")
    lines.append("- PID 控制器改为 **前馈 + 闭环 PID 修正** 架构：")
    lines.append("  - 前馈：基于 `PID_OPENLOOP_OFFSET` 与开环斜率给出基准 PWM，解决死区问题并提供快速响应；")
    lines.append("  - 闭环修正：调用 `PID_Calculate()`，根据 `target` 与高精度 `actual` 的误差计算 PWM 修正量，修正量限制在 ±300 PWM 内；")
    lines.append("  - 控制周期为 100ms（10Hz），保留 Rate Limiter、软启动、条件积分抗饱和。")
    lines.append("- PID 可调参数（含修正环 Kp/Ki/Kd、修正限幅）集中在 `main/pid.c` 顶部宏定义；")
    lines.append("- `PID_init()` 从 0 启动时强制清零 `integral` / `pre_error` / `pre_measurement`，避免上一条命令残留影响低转速启动；")
    ...
    lines.append("### 8.3 结论")
    lines.append("")
    lines.append("1. **读数精度**：`actual` 已通过 GPIO 周期捕获实现 <1 RPM 分辨率，消除了旧 PCNT 计数法的 50 RPM 量化。")
    lines.append("2. **稳态可控性**：Motor 2 在 150~4750 RPM 范围内仍保持较好稳态精度；闭环 PID 修正正在根据实际转速微调 PWM，以抑制电机自身速度波动。")
    lines.append("3. **瞬态表现**：真正从 0 启动的过冲较小；带惯性启动和切换段的\"超调\"主要是上一条命令的残余速度，通常与 MQTT broker 传输延迟或命令间隔不足有关。")
    lines.append("4. **PID 缓存问题**：在 `target=0` 时未清零 PID 的积分与历史状态会导致下一条低转速命令启动瞬间输出过高；已在代码中修复：从 0 启动时强制清零 integral / pre_error / pre_measurement。")
    lines.append("5. **脉冲异常值**：发现个别超过 5400 RPM 的单点计数，可能是脉冲噪声或电磁干扰，已从统计中剔除。")
    lines.append("6. **下一步建议**：")
    lines.append("  - 根据本次日志中的 PID 分项（err/P/I/D）以及 `ff`、`corr` 字段，进一步微调 `PID_KP` / `PID_KI` / `PID_KD`；")
    lines.append("  - 若 target=50 仍无法启动，可对 target < 100 设置独立启动 PWM 下限；")
    lines.append("  - 若稳态抖动仍明显，可增大周期捕获滑动平均窗口或降低 Kd。")
    lines.append("")
    lines.append("---")
    lines.append("")
