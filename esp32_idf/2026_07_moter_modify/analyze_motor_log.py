...
def generate_report(results, seg_results, motor_id, fig_path, transient_path):
    ...
    lines.append("## 1. 测试方法")
    lines.append("")
    lines.append("- PID 控制周期为 200ms（5Hz）。")
    ...
    lines.append("- **PID 控制周期**：保持 200ms（5Hz）。")
    lines.append("- **香农采样定理角度**：电机机械时间常数较大，速度信号带宽远低于 2.5Hz，5Hz 控制周期在理论上是足够的。")
    lines.append("- **是否提高控制频率**：")
    lines.append("  - 若目标仅为稳态精度：当前 5Hz 配合周期捕获已足够，无需改动。")
    lines.append("  - 若需进一步精细化启动/切换瞬态：可提高到 10Hz（100ms），但 PCNT 计数分辨率会下降。")
    ...
    lines.append("  - 保留条件积分抗饱和、Rate Limiter、软启动保护。")
