...
/* Write 时序 (μs) */
#define ONEWIRE_WRITE1_LOW_US       5       /**< 写 1 低电平时间 (≤15μs) */
#define ONEWIRE_WRITE1_HIGH_US      55      /**< 写 1 高电平时间，总时隙 60μs */
#define ONEWIRE_WRITE0_LOW_US       70      /**< 写 0 低电平时间 (≥60μs, ≤120μs) */
#define ONEWIRE_WRITE0_HIGH_US      5       /**< 写 0 高电平时间 */

/* Read 时序 (μs) */
#define ONEWIRE_READ_INIT_US        3       /**< 读初始化低电平时间 */
#define ONEWIRE_READ_SAMPLE_US      10      /**< 等待到采样点（总计 13μs） */
#define ONEWIRE_READ_RECOVERY_US    50      /**< 读恢复时间 */

/* 间隔时序 */
#define ONEWIRE_INTER_BIT_US        2       /**< 位间恢复时间 */
#define ONEWIRE_INTER_BYTE_US       5       /**< 字节间恢复时间 */
...
static uint32_t s_offline_check_time = 0;               /**< 离线传感器检查时间戳 */
...
/**
 * @brief 1-Wire Reset 脉冲 + Presence 检测
 * 
    ...
    *found_count = 0;
    ...
    while (!search_complete && device_count < MAX31850_SENSOR_COUNT) {
        ...
        if (err != ESP_OK || !presence) {
            ESP_LOGE(TAG, "Search ROM: No device present");
        ...
        for (uint8_t bit_pos = 0; bit_pos < 64; bit_pos++) {
            ...
            if (bit_actual == 1 && bit_complement == 1) {
                /* 无设备响应 */
                ESP_LOGW(TAG, "Search ROM: No response at bit %d", bit_pos);
                return ESP_ERR_NOT_FOUND;
        ...
        /* 验证 ROM CRC */
        if (onewire_crc8(rom_id, 7) != rom_id[7]) {
            ESP_LOGW(TAG, "Search ROM: ROM CRC error");
 ...
 * @brief 尝试恢复离线传感器
 */
static void max31850_check_offline_sensors(void)
{
    uint32_t now = xTaskGetTickCount();
    
    if ((now - s_offline_check_time) < pdMS_TO_TICKS(MAX31850_OFFLINE_RETRY_MS)) {
        return;
    }
    s_offline_check_time = now;
    ...
    for (int i = 0; i < MAX31850_SENSOR_COUNT; i++) {
        if (!s_sensors[i].online) {
            ...
            if (max31850_read_scratchpad(s_sensors[i].rom_id, data) == ESP_OK) {
                if (max31850_parse_data(data, &temp, &cj_temp, &fault) == MAX31850_OK) {
                    /* 恢复成功 */
                    if (xSemaphoreTake(s_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
                        s_sensors[i].online = true;
                        s_sensors[i].fail_count = 0;
                        s_sensors[i].thermocouple_temp = temp;
                        s_sensors[i].cold_junction_temp = cj_temp;
                        s_sensors[i].data_valid = true;
                        s_sensors[i].last_update = now;
                        xSemaphoreGive(s_mutex);
                    }
...
esp_err_t max31850_init(gpio_num_t gpio_num)
{
    ...
    if (onewire_get_level() == 0) {
        ESP_LOGE(TAG, "BUS FAULT: Line shorted to GND at startup");
