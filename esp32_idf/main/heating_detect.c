 ...
 * 每个nop约1 cycle，5个nop = 5 cycles/iteration
 * 因此每μs需要 240/5 = 48 iterations
 * 
 * @param us 延时微秒数
 */
static inline void onewire_delay_us(uint32_t us)
{
    // 240MHz: 240 cycles/μs / 5 cycles per loop = 48
...
static esp_err_t onewire_reset(bool *presence)
{
    ...
    onewire_delay_us(70);
    t_elapsed = esp_timer_get_time() - t_start;
    ESP_LOGI(TAG, "[Timing Check] Expected 70us, actual %lldus (%s)",
             t_elapsed, (t_elapsed >= 65 && t_elapsed <= 80) ? "OK" : "ERROR!");
#endif
